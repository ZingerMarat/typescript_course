...
function filterPhonesByDate<T extends IMobilePhone>(
  ...
  const initialDate: Date = new Date(initial)

  return phones
    .filter((p) => {
      const manufactured = p[key]

      return manufactured instanceof Date && manufactured > initialDate
    })
    .map((p) => {
      const newObj = { ...p, initialDate: initial }
      return newObj
    })
}
